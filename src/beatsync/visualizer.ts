import { InferenceManagerDebugData } from './schema';
import { StabilizerDebugData, StabilizerTrajectory } from "./schema";
import { ExternalClockAdjustType, ExternalClockDebugData } from "./schema";
...
export class Visualizer {
  ...
  drawBarPhaseClock(barPhase: number, bestTrajBarPhase: number, currentTime: number) {
    ...
    const beatMarkWidth = 40;
    const beatMarkLength = 65;
  ...
  drawOdf(ctx: CanvasRenderingContext2D, odfBands: Float32Array[]) {
    ...
    if (this.latestBpm > 0) {
      ...
      for (let t = beatTime; t < currentTime + ibi; t += ibi) {
        if (t > odfWindowStart) {
          const x = ((t - odfWindowStart) / BLOCK_DURATION_S) * canvasWidth;
          ctx.beginPath();
          ctx.moveTo(x, 0);
