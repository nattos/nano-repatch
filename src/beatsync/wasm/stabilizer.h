...
class Stabilizer {
public:
    Stabilizer(const StabilizerConfig& config,
    ...
    void addPrediction(double phase, double magnitude, double bpm, double currentTime);
    void resync();

    static double predictPhase(const StabilizerTrajectory& sample, double currentTime);
    static double predictBarPhase(const StabilizerTrajectory& sample, double currentTime);
