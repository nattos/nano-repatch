...
// Helper function to safely get a value from an emscripten::val, with a default value
template<typename T> T get_optional(emscripten::val const& obj, std::string const& name, T
       defaultValue) {
...
InferenceManagerConfig getInferenceManagerConfig(emscripten::val config) {
    ...
    imConfig.inferenceInterval = get_optional<int>(config, "inferenceInterval", 1);
    imConfig.targetSampleRate = get_optional<int>(config, "targetSampleRate", 44100);
    imConfig.lookbehindSamples = get_optional<int>(config, "lookbehindSamples", 2048);
    imConfig.delayCompensation = get_optional<double>(config, "delayCompensation", 0.05);
    imConfig.specSliceFraction = get_optional<double>(config, "specSliceFraction", 0.25);
    imConfig.exportDebugData = get_optional<bool>(config, "exportDebugData", false);
    return imConfig;
}

StabilizerConfig getStabilizerConfig(emscripten::val config) {
    StabilizerConfig s_config;
    s_config.proximityThreshold = get_optional<double>(config, "proximityThreshold", M_PI / 5.0);
    ...
    s_config.weightIncrement = get_optional<double>(config, "weightIncrement", 3.0);
    ...
    s_config.bestTrajectoryBias = get_optional<double>(config, "bestTrajectoryBias", 15.0);
    s_config.bpmWeightScale = get_optional<double>(config, "bpmWeightScale", 100.0);
    s_config.bpmWeightBias = get_optional<double>(config, "bpmWeightBias", -0.2);
    s_config.bpmVariancePenalty = get_optional<double>(config, "bpmVariancePenalty", 45.0);
    s_config.shiftWeight = get_optional<double>(config, "shiftWeight", 0.5);
    s_config.shiftWeightBias = get_optional<double>(config, "shiftWeightBias", 1.5);
    s_config.overcorrectionWeightThreshold = get_optional<double>(config, "overcorrectionWeightThreshold", 10.0);
    s_config.overcorrectionBpmThreshold = get_optional<double>(config, "overcorrectionBpmThreshold", 0.3);
    s_config.overcorrectionPhaseThreshold = get_optional<double>(config, "overcorrectionPhaseThreshold", M_PI / 16.0);
    s_config.bpmOvercorrectionFactor = get_optional<double>(config, "bpmOvercorrectionFactor", 0.3);
    s_config.phaseOvercorrectionFactor = get_optional<double>(config, "phaseOvercorrectionFactor", 0.3);
    s_config.deltaHistorySize = get_optional<int>(config, "deltaHistorySize", 10);
    s_config.exportDebugData = get_optional<bool>(config, "exportDebugData", false);
    return s_config;
}

ExternalClockControllerConfig getExternalClockControllerConfig(emscripten::val config) {
    ExternalClockControllerConfig ecc_config;
    ecc_config.updateInterval = get_optional<double>(config, "updateInterval", 1.0 / 30.0);
    ecc_config.largePhaseErrorThreshold = get_optional<double>(config, "largePhaseErrorThreshold", M_PI / 6.0);
    ecc_config.largeBpmDifferenceThreshold = get_optional<double>(config, "largeBpmDifferenceThreshold", 5.0);
    ecc_config.phaseCorrectionThreshold = get_optional<double>(config, "phaseCorrectionThreshold", M_PI / 8.0);
    ecc_config.predictionHorizonS = get_optional<double>(config, "predictionHorizonS", 5.0);
    ecc_config.bpmNudgeThreshold = get_optional<double>(config, "bpmNudgeThreshold", 4.0);
    ecc_config.minNudgeIntervalS = get_optional<double>(config, "minNudgeIntervalS", 1.0);
    ecc_config.bpmDriftThreshold = get_optional<double>(config, "bpmDriftThreshold", 0.5);
    ecc_config.bpmTightDriftThreshold = get_optional<double>(config, "bpmTightDriftThreshold", 0.01);
    ecc_config.bpmTightDriftIntervalS = get_optional<double>(config, "bpmTightDriftIntervalS", 2.0);
    ecc_config.bpmFilterWindowLength = get_optional<int>(config, "bpmFilterWindowLength", 5);
    ecc_config.phaseFilterWindowLength = get_optional<int>(config, "phaseFilterWindowLength", 5);
    ecc_config.exportDebugData = get_optional<bool>(config, "exportDebugData", false);
    return ecc_config;
}
...
const emscripten::val vector_to_val_float(const std::vector<float>& vec) {
    return emscripten::val::global("Float32Array").new_(emscripten::typed_memory_view(vec.size(), vec.data()));
}
      ...
      runBpmPhasePredictor_(runBpmPhasePredictor)
{
    ...
    config.inferenceConfig = getInferenceManagerConfig(configVal["inferenceConfig"]);
    config.stabilizerConfig = getStabilizerConfig(configVal["stabilizerConfig"]);
    config.externalClockControllerConfig = getExternalClockControllerConfig(configVal["externalClockControllerConfig"]);
    config.exportAllDebugData = get_optional<bool>(configVal, "exportAllDebugData", false);

    // Create std::function wrappers for the emscripten::val callbacks
    auto onPredictionReadyProxy = [this](const BpmPhasePredictorResult& result, const InferenceManagerDebugData& debugData) {
        ...
        emscripten::val outOdfWindow = emscripten::val::array(outOdfWindowChannels.begin(), outOdfWindowChannels.end());
        ...
        emscripten::val outSpecWindow = emscripten::val::array(outSpecWindowChannels.begin(), outSpecWindowChannels.end());
    ...
    auto onRequestFeatureExtractionProxy = [this](const std::vector<float>& audio) {
        emscripten::val audioVal = vector_to_val_float(audio);
        runFeatureExtractor_(audioVal);
    };

    auto onRequestBpmPhasePredictionProxy = [this](const std::vector<float>& odf, const std::vector<float>& spec, double inputTime) {
    ...
    auto onBestTrajectoryProxy = [this](const StabilizerTrajectory& bestTrajectory) {
        // This callback was originally used to update externalClockController_.
        // Now, the core AudioToClock handles this internally.
        // If there's a need to expose this to JS, it should be done via onDebugDataExported_
        // or a new specific callback. For now, I'll assume it's handled internally.
    };
    ...
    auto onStabilizerDebugDataProxy = [this](const StabilizerDebugData& data) {
        ...
        stabilizer.set("trajectories", emscripten::val::array(trajectories.begin(), trajectories.end()));
    ...
    auto onExternalClockAdjustedProxy = [this](const ExternalClockAdjustEvent& event) {
        emscripten::val changes = emscripten::val::object();
        if (event.bpm) {
            changes.set("bpm", event.bpm.value());
        }
        if (event.phase) {
            changes.set("phase", event.phase.value());
        }
    ...
    auto onExternalClockDebugDataProxy = [this](const ExternalClockDebugData& data, const std::optional<ExternalClockAdjustEvent>& event) {
        ...
        externalClock.set("phase", data.phase);
        externalClock.set("barPhase", data.barPhase);
        if (data.scheduledBpmCorrection.has_value()) {
            ...
            correction.set("scheduledAt", data.scheduledBpmCorrection.value().scheduledAt);
    ...
    audioToClockCore_ = std::make_unique<AudioToClock>(
        config,
        onPredictionReadyProxy,
        onRequestFeatureExtractionProxy,
        onRequestBpmPhasePredictionProxy,
        onBestTrajectoryProxy,
        onStabilizerDebugDataProxy,
        onExternalClockAdjustedProxy,
...
void AudioToClockWasm::addAudio(uintptr_t audioSamplesPtr, int numChannels, int numSamples, double currentTime, int inputSampleRate) {
    audioToClockCore_->addAudio(audioSamplesPtr, numChannels, numSamples, currentTime, inputSampleRate);
}

void AudioToClockWasm::resolveFeatureExtractor(uintptr_t odfDataPtr, size_t odfLength, uintptr_t specDataPtr, size_t specLength) {
    audioToClockCore_->resolveFeatureExtractor(odfDataPtr, odfLength, specDataPtr, specLength);
}
...
EMSCRIPTEN_BINDINGS(AudioToClockModule) {
    emscripten::class_<AudioToClockWasm>("AudioToClock") // Bind the new wrapper class
        .constructor<emscripten::val, emscripten::val, emscripten::val, emscripten::val,
       emscripten::val, emscripten::val>()
        .function("addAudio", &AudioToClockWasm::addAudio, emscripten::allow_raw_pointers())
        .function("tick", &AudioToClockWasm::tick)
        .function("resolveFeatureExtractor", &AudioToClockWasm::resolveFeatureExtractor, emscripten::allow_raw_pointers())
        .function("resolveBpmPhasePredictor", &AudioToClockWasm::resolveBpmPhasePredictor);
}