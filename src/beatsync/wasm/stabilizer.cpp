...
// ... (utility functions like wrap, wrapSigned, etc. would go here or in a utils file)
...
public:
    ...
    TrajectoryState(double p, double bp, double m, double b, double w, double lut) :
        id(nextId++), phase(p), barPhase(bp), magnitude(m), bpm(b), weight(w), lastUpdateTime(lut) {}

    double predictPhase(double currentTime) const {
        return ::predictPhase({id, phase, barPhase, magnitude, bpm, weight, lastUpdateTime}, currentTime);
    }

    double predictBarPhase(double currentTime) const {
        return ::predictBarPhase({id, phase, barPhase, magnitude, bpm, weight, lastUpdateTime}, currentTime);
    }

    void update(double newPhase, double newBarPhase, double newMagnitude, double newBpm, double weightIncrement, double currentTime, double shiftWeight, const StabilizerConfig& config);
...
void TrajectoryState::update(double newPhase, double newBarPhase, double newMagnitude, double newBpm, double weightIncrement, double currentTime, double shiftWeight, const StabilizerConfig &config) {
    ...
    // --- Phase Update ---
    double phaseDiff = std::atan2(std::sin(newPhase - oldPhase), std::
                                                                     cos(newPhase - oldPhase));
    double avgPhaseDelta = std::accumulate(phaseDeltaHistory.begin(),
                                           phaseDeltaHistory.end(), 0.0) /
                           (phaseDeltaHistory.size() ? phaseDeltaHistory.size() : 1);
    bool isPhaseDirectionConsistent = phaseDeltaHistory.empty() ||
                                      (phaseDiff > 0) == (avgPhaseDelta > 0);

    bool shouldOvercorrectPhase = weight >
                                      config.overcorrectionWeightThreshold &&
                                  std::abs(oldBpm - newBpm) <
                                      config.overcorrectionBpmThreshold &&
                                  std::abs(phaseDiff) <
                                      config.overcorrectionPhaseThreshold &&
    ...
    double avgSinPhase = std::sin(oldPhase) * biasedWeight + std::sin(targetPhase) * bpmPhaseWeightIncrement;
    double avgCosPhase = std::cos(oldPhase) * biasedWeight + std::cos(targetPhase) * bpmPhaseWeightIncrement;
    ...
    double appliedPhaseDelta = std::fmod(updatedPhase - oldPhase +
                                             M_PI,
                                         2 * M_PI) -
                               M_PI;
    ...
    double avgBpmDelta = std::accumulate(bpmDeltaHistory.begin(),
                                         bpmDeltaHistory.end(), 0.0) /
                         (bpmDeltaHistory.size() ? bpmDeltaHistory.size() : 1);
    bool isBpmDirectionConsistent = bpmDeltaHistory.empty() || (bpmDiff > 0) == (avgBpmDelta > 0);

    bool shouldOvercorrectBpm = weight >
                                    config.overcorrectionWeightThreshold &&
                                std::abs(bpmDiff) <
                                    config.overcorrectionBpmThreshold &&
    ...
    if (biasedWeight + bpmPhaseWeightIncrement > 0) {
        double updatedBpm = (oldBpm * biasedWeight + targetBpm *
                                                         bpmPhaseWeightIncrement) /
    ...
    if (newWeight > 0) {
        this->magnitude = (this->magnitude * this->weight + newMagnitude * weightIncrement) / newWeight;
    }
    this->weight = std::min(config.maxWeight, newWeight);
    this->lastUpdateTime = currentTime;
}

Stabilizer::Stabilizer(const StabilizerConfig& config,
    ...
    : config_(config), onTrajectoryUpdated_(onTrajectoryUpdated), onDebugDataUpdated_(onDebugDataUpdated) {}

Stabilizer::~Stabilizer() {}

void Stabilizer::resync() {
    for (auto& trajectory : trajectories_) {
        trajectory.barPhase = fmod(trajectory.barPhase - floor(trajectory.barPhase + 0.5), 4.0);
...
void Stabilizer::addPrediction(double phase, double magnitude, double bpm, double currentTime) {
    ...
    overallConfidence_ = std::min(1.0, std::max(0.0,
       config_.bpmWeightScale / (1.0 + config_.bpmVariancePenalty *
       lastBpmVariance_) + config_.bpmWeightBias));
    double modulatedWeightIncrement = config_.weightIncrement *
       overallConfidence_;

    bool foundMatch = false;
    double bestBarPhase = bestTrajectory_ ? bestTrajectory_->
       predictBarPhase(currentTime) : 0.0;
    for (auto& trajectory : trajectories_) {
        ...
        if (phaseDifference < config_.proximityThreshold) {
            trajectory.update(phase, bestBarPhase, magnitude, bpm,
                modulatedWeightIncrement, currentTime, config_.shiftWeight, config_);
            foundMatch = true;
            break;
        }
    }
    ...
    if (!foundMatch) {
        double averageBpm = std::accumulate(bpmHistory_.begin(),
        bpmHistory_.end(), 0.0) / bpmHistory_.size();
        ...
        trajectories_.emplace_back(phase, newBarPhase, magnitude,
            averageBpm, config_.initialWeight * overallConfidence_, currentTime);
    }

    decayAndPrune(currentTime);
    findBestTrajectory(currentTime);

    if (config_.exportDebugData) {
...
void Stabilizer::SetForceExportAllDebugData(bool forceExport) {
    config_.exportDebugData = forceExport;
}
...
void Stabilizer::doExportDebugData(double currentTime) {
    ...
    onDebugDataUpdated_({trajectories, bestTrajectory_copy, hasBestTrajectory, overallConfidence_, lastBpmVariance_, bpmHistory_});
}
...
void Stabilizer::decayAndPrune(double currentTime) {
    ...
    trajectories_.erase(std::remove_if(trajectories_.begin(),
       trajectories_.end(),
...
double Stabilizer::predictPhase(const StabilizerTrajectory& sample, double currentTime) {
...
double Stabilizer::predictBarPhase(const StabilizerTrajectory& sample, double currentTime) {
  const double timeSinceUpdate = currentTime - sample.lastUpdateTime;
  const double ibi = 60.0 / sample.bpm;
