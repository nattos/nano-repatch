...
ExternalClockController::ExternalClockController(
    ...
    : config_(config), onClockAdjusted_(onClockAdjusted), onDebugDataUpdated_(onDebugDataUpdated) {}
...
void ExternalClockController::update(double predictedBpm, double
      predictedPhase, double predictedBarPhase, double currentTime) {
    ...
    double averageBpm = std::accumulate(bpmFilterHistories_.begin(),
      bpmFilterHistories_.end(), 0.0) / bpmFilterHistories_.size();
    ...
    for (const auto& obs : phaseFilterHistories_) {
        ...
        double phaseAdvance = (timeDiff * averageBpm / 60.0) * 2 *
      M_PI;
...
void ExternalClockController::tick(double currentTime) {
    ...
    if (lastUpdateTime_ == 0) {
        ...
        onClockAdjusted_({externalBpm_, externalPhase_, currentTime, ExternalClockAdjustType::Sync});
        return;
    }
    ...
    externalBarPhase_ = std::round(externalBarPhase_ + tickPhaseAdvance / M_PI / 2.0 - externalPhase01) + externalPhase01;
    ...
    const double futureInternalPhase = predictedPhase + (predictionHorizonS * predictedBpm / 60.0) * 2 * M_PI;

    double predictionPhaseAdvance;
    const double dtCorrection = scheduledBpmCorrection_ ? scheduledBpmCorrection_->time - currentTime : -1;

    if (scheduledBpmCorrection_ && dtCorrection > 0 && dtCorrection < predictionHorizonS) {
      const double currentBpmPhaseAdvance = dtCorrection * externalBpm_;
      const double futureBpmPhaseAdvance = (predictionHorizonS - dtCorrection) * scheduledBpmCorrection_->bpm;
      predictionPhaseAdvance = (currentBpmPhaseAdvance + futureBpmPhaseAdvance) / 60.0 * 2 * M_PI;
    } else {
      predictionPhaseAdvance = (predictionHorizonS * externalBpm_ / 60.0) * 2 * M_PI;
    }
    const double futureExternalPhase = externalPhase_ + predictionPhaseAdvance;
    const double phaseError = wrapSigned(futureInternalPhase - futureExternalPhase, M_PI);

    // 4. Decide on correction
    const double bpmNudgeRequired = (phaseError * 60.0) / (2.0 * M_PI * config_.predictionHorizonS);

    if ((std::abs(phaseError) > config_.largePhaseErrorThreshold && std::abs(bpmNudgeRequired) > config_.bpmNudgeThreshold) ||
        std::abs(predictedBpm - externalBpm_) > config_.largeBpmDifferenceThreshold) {
      // Hard reset
      externalBpm_ = predictedBpm;
      externalPhase_ = predictedPhase;
      lastBpmChangeTime_ = currentTime;
    ...
    } else if ((!scheduledBpmCorrection_ || (currentTime - scheduledBpmCorrection_->scheduledAt) > config_.minNudgeIntervalS) && std::abs(phaseError) > config_.phaseCorrectionThreshold) {
      ...
      if (timeSinceLastBpmChange > config_.bpmTightDriftIntervalS && std::abs(bpmDifference) > config_.bpmTightDriftThreshold) {
        // Snap BPM
        externalBpm_ = predictedBpm;
        lastBpmChangeTime_ = currentTime;
        didUpdateWithType = ExternalClockAdjustType::Nudge;
    ...
    if (didUpdateWithType) {
        ...
        eventToReport = ExternalClockAdjustEvent{bpmToReport, phaseToReport, currentTime, didUpdateWithType.value()};
...
void ExternalClockController::doExportDebugData(const std::optional<ExternalClockAdjustEvent>& event) {
...
float ExternalClockController::getBarPhase() const {
    return externalBarPhase_;
}
...
void ExternalClockController::resync() {
    ...
    externalBarPhase_ += 4.0;
    latestPredictedBarPhase_ = latestPredictedBarPhase_ - floor(latestPredictedBarPhase_ + 0.5);
