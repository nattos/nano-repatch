...
public:
  AudioToClockWasm(emscripten::val config, emscripten::val onStatusUpdated,
  ...
  void addAudio(uintptr_t audioSamplesPtr, int numChannels, int numSamples,
                double currentTime, int inputSampleRate);
  void tick(double currentTime);

  void resync(bool hard);
  void resetHardSync();
  void setForceExportAllDebugData(bool force);

  void resolveFeatureExtractor(uintptr_t odfDataPtr, size_t odfLength,
                               uintptr_t specDataPtr, size_t specLength);
