...
InferenceManager::InferenceManager(const InferenceManagerConfig& config,
    ...
    : config_(config), onPrediction_(onPrediction), requestFeatureExtractor_(requestFeatureExtractor), requestBpmPhasePredictor_(requestBpmPhasePredictor),
...
void InferenceManager::addAudio(const std::vector<std::vector<float>>& channels, double currentTime, int inputSampleRate) {
    ...
    if (!audioBlocks_.empty()) {
        ...
        double expectedTime = lastBlock.timestamp + lastBlock.data.size() / (double)config_.targetSampleRate;
    ...
    if (audioBlocks_.size() > MAX_QUEUED_BLOCKS) {
        audioBlocks_.erase(audioBlocks_.begin(), audioBlocks_.begin() + (audioBlocks_.size() - MAX_QUEUED_BLOCKS));
    }
}
...
void InferenceManager::process() {
    ...
    pending_chunk_ = getAudioChunk(numHopsToProcess_ * hopSamples + lookbehindSamples);
    consumeAudio(numHopsToProcess_ * hopSamples);

    std::vector<float> float_chunk_data(pending_chunk_.data.begin(), pending_chunk_.data.end());
...
void InferenceManager::processHops() {
    ...
    const int lookbehindOdfFrames = std::floor(lookbehindSamples / HI_RES_HOP_LENGTH);
    const int lookbehindSpecFrames = std::floor(lookbehindSamples / LOW_RES_HOP_LENGTH);
    const double hopTime = config_.hopSamples / (double)config_.targetSampleRate;

    for (int i = currentHop_; i < numHopsToProcess_; ++i) {
        currentHop_ = i;
        const int odfFramesFor_i_hops = std::floor(i * config_.hopSamples / HI_RES_HOP_LENGTH);
        const int odfFramesFor_i_plus_1_hops = std::floor((i + 1) * config_.hopSamples / HI_RES_HOP_LENGTH);
        const int hopOdfFramesCount = odfFramesFor_i_plus_1_hops - odfFramesFor_i_hops;

        const int specFramesFor_i_hops = std::floor(i * config_.hopSamples / LOW_RES_HOP_LENGTH);
        const int specFramesFor_i_plus_1_hops = std::floor((i + 1) * config_.hopSamples / LOW_RES_HOP_LENGTH);
        const int hopSpecFramesCount = specFramesFor_i_plus_1_hops - specFramesFor_i_hops;
        ...
        if (hopOdfFramesCount > 0) {
            ...
            for (int b = 0; b < ODF_CHANNELS; ++b) {
                ...
                const float* newFrames = planar_features.odf.data() + b * odf_frames_wide + odfFrameStart;
                float* newBand = newOdfWindow.data() + b * config_.odfFrames;
                std::copy(oldBand + hopOdfFramesCount, oldBand + config_.odfFrames, newBand);
                std::copy(newFrames, newFrames + hopOdfFramesCount, newBand + config_.odfFrames - hopOdfFramesCount);
            }
        ...
        if (hopSpecFramesCount > 0) {
            ...
            for (int b = 0; b < SPEC_CHANNELS; ++b) {
                ...
                const float* newFrames = planar_features.spec.data() + b * spec_frames_wide + specFrameStart;
                float* newBand = newSpecWindow.data() + b * config_.specFrames;
                std::copy(oldBand + hopSpecFramesCount, oldBand + config_.specFrames, newBand);
                std::copy(newFrames, newFrames + hopSpecFramesCount, newBand + config_.specFrames - hopSpecFramesCount);
            }
        ...
        if (hopCounter_ % config_.inferenceInterval == 0) {
            ...
            if (specSliceFraction < 1.0) {
                ...
                if (slicedFrames < originalFrames) {
                    ...
                    for (int j = 0; j < SPEC_CHANNELS; ++j) {
                        const float* channel_start = specWindow_.data() + j * originalFrames;
                        const float* slice_start = channel_start + (originalFrames - slicedFrames);
            ...
            double predictionTime = pending_chunk_.timestamp + i * hopTime - config_.delayCompensation - config_.lookbehindSamples / (double)config_.targetSampleRate;
...
void InferenceManager::onPredictionReady(const BpmPhasePredictorResult& result) {
    // assert(state_ == State::ExpectingPrediction);

    if (this->config_.exportDebugData) {
...
AudioBlock InferenceManager::getAudioChunk(size_t numSamples) {
    ...
    for (const auto& block : audioBlocks_) {
        size_t samplesToCopy = std::min(numSamples - samplesCopied, block.data.size());
        audioChunk.insert(audioChunk.end(), block.data.begin(), block.data.begin() + samplesToCopy);
...
void InferenceManager::consumeAudio(size_t numSamples) {
    ...
    while (samplesToConsume > 0 && !audioBlocks_.empty()) {
        ...
        if (samplesToConsume >= block.data.size()) {
            ...
            block.data.erase(block.data.begin(), block.data.begin() + samplesToConsume);
        ...
        if (this->odfWindow_.size() >= (config_.odfFrames * ODF_CHANNELS)) {
            ...
            for (int i = 0; i < ODF_CHANNELS; ++i) {
                for (int j = 0; j < odfFrames; ++j) {
                    this->debugData_.odfWindow[i][j] = this->odfWindow_[i * odfFrames + j];
                }
            }
        }
        ...
        if (this->specWindow_.size() >= (config_.specFrames * SPEC_CHANNELS)) {
            ...
            for (int i = 0; i < SPEC_CHANNELS; ++i) {
                for (int j = 0; j < specFrames; ++j) {
                    this->debugData_.specWindow[i][j] = this->specWindow_[i * specFrames + j];
                }
            }
        }
    }
