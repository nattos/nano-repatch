...
describe('Long Vertical Wire Test', () => {
    beforeAll(async () => {
        await page.goto(URL);
    ...
    it('connects Hub(0,0) to Hub(0,2) with a continuous vertical wire', async () => {
        // 1. Create Nodes
        const hub1 = await createNode('util.hub', 0, 0);
        const hub2 = await createNode('util.hub', 0, 2); // Gap of 1 row (Row 1 is empty)

        // 2. Connect Hub1 (Index 0) to Hub2 (Index 0)
        await createConnection(hub1, hub2, 'value', 'value');
